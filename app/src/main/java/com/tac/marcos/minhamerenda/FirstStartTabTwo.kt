...
class FirstStartTabTwo : AppCompatActivity() {
    ...
    private var requestTwo: Request? = null

    private val http = OkHttpClass()
    private val urlEscola: String = "minha-merenda.herokuapp.com/ts830/list/escola"
    private val urlLocations: String = "minha-merenda.herokuapp.com/ts830/list/geometry"

    private val urlAvaliacao: String = "minha-merenda.herokuapp.com/ts830/list/avaliacao"

    //CLASSES TO MAP
    private var escolaList = ArrayList<Escola>()
    private val escolaListAux = ArrayList<Escola>()
...
//    private var avaliacaoToSend: Avaliacao? = null
    ...
    private var jsonTwo: String? = null

    private var jsonresp = JSONArray()
    private var jsonrespLocations = JSONArray()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        latitudeAtual = bundle.getDouble("latitude", 0.0)
        longitudeAtual = bundle.getDouble("longitude", 0.0)
        ...
        thread {
            ...
            try{
                ...
                requestTwo = http.getRequest(urlLocations)

                jsonTwo = http.GETurl(client, requestTwo)
                ...
                this.runOnUiThread {
                    ...
                    try {
                        ...
                        val escolaLatitude = ArrayList<Double>()
                        val escolaLongitude = ArrayList<Double>()

                        jsonresp = org.json.JSONArray(json)
                        jsonrespLocations = org.json.JSONArray(jsonTwo)

                        escolaListAux.clear()
                        for (i in 0 until jsonresp.length()) {
                            var r = Escola()
                            Log.i("sttttttt", jsonresp.toString())
                            ...
                            r.setLatitude(jsonrespLocations.getJSONObject(i).getString("x"))
                            escolaLatitude.add(jsonrespLocations.getJSONObject(i).getDouble("x"))
                            r.setLongitude(jsonrespLocations.getJSONObject(i).getString("y"))
                            escolaLongitude.add(jsonrespLocations.getJSONObject(i).getDouble("y"))
                            Log.i("xxxyyy =",  jsonrespLocations.getJSONObject(i).getString("x"))

                            //Escolas sem distancia

                            escolaListAux.add(r)
                        }

                        var LH = LocationHelper()
                        //esccolas com distancias

                        val transforma1 = LH.transforma(escolaListAux, latitudeAtual, longitudeAtual)
                        //Escolas ordenadas
                        escolaList = LH.ordenar(transforma1)
                        ...
                        for (i in escolaList){
                            ...
                            Log.i("mmpppsss", i.getLatitude().toString())
